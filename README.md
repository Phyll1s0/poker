...
- 界面对局与无头自博弈共用牌型、权益特征和“均衡锚定 → 风格表达 → 有界动态反制”决策核心，可用固定种子做数万手强度回归
...
- **单手训练**：每手结束立即给出点评，但筹码和各座位的隐藏 AI 风格会继续保留；玩家低于 1 BB 时才按当前桌型自动补充，只有重新开始训练或切换桌型才会重置。适合边打边学，同时观察连续牌局中的筹码深度和对手调整。
...
所有电脑现在都从同一份本地平衡策略出发，再表达有限风格。动态反制优先使用匹配街道、行动族、尺度、位置、人数与筹码深度的公开响应节点；细节点样本不足时，全局松紧/侵略性画像只作低权重泛化。单次决策对 `fold/check/call/raise` 四类动作混合设有 TV 上限；它不约束完整下注尺度联合分布，也不是全局 exploitability 证书。动态型预算最大，GTO 型最克制，其他三种也不会对持续偷池失明。可单独运行：

```bash
npm run ai:benchmark:adaptation
```

它会在固定的合成公开画像下审计河牌过弃、跟注站和持续开池三类倾向的最终行动频率与实际抽样；这是确定性的策略回归，不会在命令运行时训练或更新电脑参数。经验画像、五种风格预算、论文启发与实现边界见 [`ADAPTIVE_AI.md`](ADAPTIVE_AI.md)。
