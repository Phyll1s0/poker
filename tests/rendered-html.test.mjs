...
test("keeps the multiplayer and strategy boundaries with product metadata", async () => {
  ...
  assert.match(rulesModal, /不是比“两张底牌”，而是比最佳五张/);
  assert.match(rulesModal, /九种牌型，从大到小/);
  assert.match(rulesModal, /也叫皇家同花顺/);
  assert.match(rulesModal, /单挑时庄家兼任小盲/);
  assert.match(rulesModal, /下一次完整最小加注就是至 50/);
  ...
  assert.doesNotMatch(pokerAudio, /SpeechSynthesisUtterance|speechSynthesis/);
  assert.match(pokerAudio, /renderMultiplayerReactionVoice/);
  ...
  assert.match(multiplayerClient, /aria-label=\{soundOn \? "关闭牌桌与表情音效" : "开启牌桌与表情音效"\}/);
