...
- **两人通用 CFR+ 核心**：已通过 Kuhn 已知值，以及与 CFR 更新逻辑分离的穷举 best response 测试。
- **两人河牌**：目前只有上面这一项公开独立同题交叉验证。它提高了对实现正确性的信心，但样本数仍是 1。
...
1. 增加更多不同牌面、范围不对称、价值/诈唬结构和尺度树的 `noambrown` 固定基线。
2. 用 OpenSpiel 的小博弈与另一套 best-response 实现持续做算法回归。
